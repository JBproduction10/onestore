...
export const getRelatedProducts = async (
  ...
  let relatedProducts: ProductWithVariants[] = subCategoryProducts as ProductWithVariants[];
  ...
  const productsWithFilteredVariants = relatedProducts.map((product: ProductWithVariants) => {
    ...
    const variants: VariantSimplified[] = filteredVariants.map((variant: typeof filteredVariants[0]) => ({
    ...
    const variantImages: VariantImageType[] = filteredVariants.map(
      (variant: typeof filteredVariants[0]) => ({
