...
export const getHomeFeaturedCategories = async () => {
  ...
  return featuredCategories.map((category: typeof featuredCategories[0]) => ({
    id: category.id,
    name: category.name,
    url: category.url,
    productCount: category._count.products,
    subCategories: category.subCategories.map((subcategory: typeof category.subCategories[0]) => ({
      id: subcategory.id,
