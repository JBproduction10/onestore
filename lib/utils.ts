...
export async function getUserCountry(req: VercelRequest): Promise<Country> {
  let userCountry: Country = DEFAULT_COUNTRY;

  // If geo data is available (in production on Vercel as an edge function)
  const geo = req.geo; // For edge functions in Vercel
  if (geo) {
    userCountry = {
    ...
    try {
      ...
      if (response.ok) {
        ...
        userCountry = {
          name:
            countries.find((c: CountryType) => c.code === data.country)?.name ||
            data.country ||
